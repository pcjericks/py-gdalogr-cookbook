...
This recipe creates a fishnet grid. Input arguments are:
``output-shapefile-name.shp, xmin, xmax, ymin, ymax, gridHeight, gridWidth``
...
.. code-block:: python
    ...
    def main(outputGridfn,xmin,xmax,ymin,ymax,gridHeight,gridWidth):
        ...
        while countcols < cols:
            ...
            ring = ogr.Geometry(ogr.wkbLinearRing)
            ring.AddPoint(ringXleftOrigin, ringYtopOrigin)
            ring.AddPoint(ringXrightOrigin, ringYtopOrigin)
            ring.AddPoint(ringXrightOrigin, ringYbottomOrigin)
            ring.AddPoint(ringXleftOrigin, ringYbottomOrigin)
            poly = ogr.Geometry(ogr.wkbPolygon)
            poly.AddGeometry(ring)
